...
3. 助手在什么场景应自己处理，什么场景应通过团队协作把任务交给 `Codex`，什么场景必须让人确认。
...
flowchart LR
    ...
    A --> T["本地工具<br/>memory_search / session_search / execute_command / create_teammate / message_teammate"]
    T --> A
```
...
3. **编排**：判断应直接回答、调用本地工具、还是创建/使用 `Codex` 队友继续协作。
...
3. **再判断是否应交给 `Codex` 队友协作**
   - 涉及复杂代码修改、长链执行、跨文件重构、工程构建/测试、复杂协议调用时，优先创建 `Codex` 队友；一次性任务优先 `persistence=temporary`，长期协作优先 `persistence=persistent`。
4. **最后判断是否必须让人确认**
...
出现以下情况时，助手应优先考虑通过 Team 模型把任务交给 `Codex`：
...
2. 助手能明确说明“这件事我自己做 / 我交给 Codex 队友处理 / 需要你确认”的原因。
