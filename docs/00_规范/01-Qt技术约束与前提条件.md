...
| Skill 库 | YAML 文件 | 人类可读可编辑，适合技能定义与维护 |
...
### 约束 19：QChatWidget 组件的多实例支持

愿景中每个 Tab 都有自己的 ChatListWidget 和 ChatWidget。当前 QChatWidget 组件的情况：
...
如果未来需要升级 Qt 版本，需要评估 QChatWidget 组件与 qtkeychain 的兼容性。
...
项目资源目录中有 `resources/openssl/` 目录，说明 HTTPS 请求依赖 OpenSSL 动态库：
- 所有 LLM API 调用都是 HTTPS，OpenSSL 是硬依赖
- 插件如果需要 HTTPS（如 Telegram Bot API），也依赖同一套 OpenSSL
- 部署时必须携带 `libssl-1_1.dll` 和 `libcrypto-1_1.dll`（或对应兼容版本）
...
*文档版本：v1.2*
*创建日期：2026-02-08*
*关联文档：docs/00_规范/00-项目愿景与产品规划.md、docs/00_规范/02-愿景落地路线图.md、docs/10_方案/10-架构升级设计方案.md*
