...
bool LspTool::isCppFile(const QString& filePath)
{
    QString ext = "." + QFileInfo(filePath).suffix().toLower();
    return ext == ".cpp" || ext == ".h" || ext == ".hpp" || ext == ".cc" || ext == ".cxx";
}

void LspTool::scheduleRetry(const QJsonObject& input, const QString& toolCallId)
{
    if (toolCallId.isEmpty())
        return;
...
QString LspTool::formatLocations(const QList<Lsp::Location>& locs)
{
    if (locs.isEmpty())
        return "未找到结果";
    QString res = "找到结果:\n";
