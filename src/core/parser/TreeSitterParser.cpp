...
QString SyntaxNode::text() const
{
    ...
    if (start >= static_cast<uint32_t>(source.size()) || end > static_cast<uint32_t>(source.size()) || start > end) {
        return QString();
    }

    return QString::fromUtf8(source.mid(static_cast<int>(start), static_cast<int>(end - start)));
}
...
SyntaxNode SyntaxNode::childByFieldName(const QString& name) const
{
    ...
    TSNode child = ts_node_child_by_field_name(node, utf8.constData(), static_cast<uint32_t>(utf8.size()));
    return SyntaxNode::fromInternal(&child, m_parser);
}
...
bool TreeSitterParser::parse(const QByteArray& utf8Source)
{
    ...
    m_source = utf8Source;
    m_tree = ts_parser_parse_string(m_parser, nullptr, m_source.constData(), static_cast<uint32_t>(m_source.size()));
...
void TreeSitterParser::applyEdit(uint32_t startByte, uint32_t oldEndByte, uint32_t newEndByte, uint32_t startRow, uint32_t startCol, uint32_t oldEndRow, uint32_t oldEndCol, uint32_t newEndRow, uint32_t newEndCol)
{
    if (!m_tree) {
...
bool TreeSitterParser::reparse(const QByteArray& newUtf8Source)
{
    ...
    m_tree = ts_parser_parse_string(m_parser, treeForParsing, m_source.constData(), static_cast<uint32_t>(m_source.size()));
