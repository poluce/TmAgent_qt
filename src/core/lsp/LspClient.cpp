...
bool LspClient::start(const QString& rootPath)
{
    ...
    connect(m_transport, &JsonRpcTransport::messageReceived, this, &LspClient::onMessageReceived);
    connect(m_transport, &JsonRpcTransport::errorOccurred, this, &LspClient::onTransportError);
...
void LspClient::sendInitialize()
{
    ...
    workspaceFolders.append(QJsonObject { { "uri", Lsp::pathToUri(m_rootPath) }, { "name", QFileInfo(m_rootPath).fileName() } });
...
void LspClient::notifyDidOpen(const QString& filePath, const QString& text, const QString& languageId)
{
    if (!isReady())
        return;
...
void LspClient::sendLocationRequest(const QString& method, const QString& filePath, int line, int character, LocationCallback callback, const QJsonObject& extraParams)
{
    if (!isReady())
        return;
    ...
    m_transport->sendRequest(method, params, id);
}

// 通用位置请求：definition / implementation / references 共用
void LspClient::requestDefinition(const QString& filePath, int line, int character, LocationCallback callback)
{
    sendLocationRequest("textDocument/definition", filePath, line, character, callback);
}

void LspClient::requestReferences(const QString& filePath, int line, int character, LocationCallback callback)
{
    sendLocationRequest("textDocument/references", filePath, line, character, callback, { { "context", QJsonObject { { "includeDeclaration", true } } } });
}

void LspClient::requestHover(const QString& filePath, int line, int character, HoverCallback callback)
{
    if (!isReady())
        return;
...
void LspClient::requestImplementation(const QString& filePath, int line, int character, LocationCallback callback)
{
    sendLocationRequest("textDocument/implementation", filePath, line, character, callback);
}

void LspClient::requestPrepareCallHierarchy(const QString& filePath, int line, int character, CallHierarchyCallback callback)
{
    if (!isReady())
        return;
