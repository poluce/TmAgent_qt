...
JsonRpcTransport::JsonRpcTransport(QProcess* process, QObject* parent)
    ...
    , m_process(process)
{
    connect(m_process, &QProcess::readyReadStandardOutput, this, &JsonRpcTransport::onReadyRead);
    connect(m_process, &QProcess::errorOccurred, this, &JsonRpcTransport::onProcessError);
}
...
bool JsonRpcTransport::tryParseMessage()
{
    ...
    if (m_expectedLength < 0) {
        ...
        static QRegularExpression contentLengthRegex("Content-Length:\\s*(\\d+)", QRegularExpression::CaseInsensitiveOption);
