...
QList<Tool> buildMemoryToolSchemas()
{
    return {
        makeToolSchema(
...
MemoryToolProvider::MemoryToolProvider(QObject* parent)
    : QObject(parent)
    , m_tools(buildMemoryToolSchemas())
{
}

QList<Tool> MemoryToolProvider::toolSchemas()
{
    return buildMemoryToolSchemas();
}

QList<Tool> MemoryToolProvider::listTools() const
{
    return m_tools;
}

ToolResult MemoryToolProvider::execute(const ToolCall& call)
{
