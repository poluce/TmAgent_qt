...
void show(QWidget* parent, ChatService* chatService)
{
    ...
    editor->setPlainText(chatService->loadMcpConfigSpecs().join('\n'));
    ...
    QObject::connect(buttons, &QDialogButtonBox::accepted, &dlg, [=, &dlg]() {
        ...
        if (!chatService->saveMcpConfigSpecs(newSpecs)) {
            QMessageBox::warning(parent, QObject::tr("保存失败"), QObject::tr("无法写入 MCP 配置文件。"));
            return;
        }

        chatService->applyMcpConfig(newSpecs);
        chatService->applyToolDispatcherToAllRuntimes();
