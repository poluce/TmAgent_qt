...
QJsonArray runtimeIoHistoryForSession(ChatService* chatService, const QString& sessionId)
{
    if (!chatService || sessionId.trimmed().isEmpty())
        return QJsonArray();

    AgentRuntime* runtime = chatService->runtimeForSession(sessionId);
    if (runtime && runtime->currentSessionId() == sessionId)
        return runtime->getIoHistory();
    return QJsonArray();
}

void clearConversationHistory(ChatService* chatService, const QString& sessionId)
{
    ...
    AgentRuntime* runtime = chatService ? chatService->runtimeForSession(sessionId) : nullptr;
    if (runtime && runtime->currentSessionId() == sessionId)
