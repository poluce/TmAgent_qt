...
#include "core/service/ConfigService.h"
...
void show(QWidget* parent, ChatService* chatService, const QString& activeIdentityId)
{
    ...
    ConfigService* configService = chatService->configService();
    const bool canManageGlobalConfig = chatService->canIdentityManageGlobalConfig(activeIdentityId);
    ...
    auto reloadMemorySettingsUi = [&]() {
        ...
        const QJsonObject policyObj = configService ? configService->loadMemoryPolicyObject() : QJsonObject();
        ...
        const QStringList configIds = chatService && chatService->modelFactory()
            ? chatService->modelFactory()->registeredConfigIds()
            : QStringList();
        ...
        if (stewardConfigId.isEmpty() && chatService)
            stewardConfigId = ModelFactory::resolveConfigKey(chatService->defaultAgentConfig());
        ...
        const QString userMd = configService ? configService->loadUserMemoryMarkdown() : QString();
    ...
    auto saveMemorySettingsUi = [&](QString* error) -> bool {
        ...
        QJsonObject policyObj = configService->loadMemoryPolicyObject();
        ...
        if (!configService->saveMemoryPolicyObject(policyObj)) {
            if (error)
        ...
        if (!configService->saveUserMemoryMarkdown(userMarkdown, &userSaveError)) {
            if (error)
                *error = userSaveError.isEmpty() ? QObject::tr("写入 user.md 失败") : userSaveError;
            return false;
        }

        chatService->applyConfigToAllRuntimes();
    ...
    auto onMemoryStewardChanged = [&]() {
        if (memoryUiLoading)
            return;
        const QString stewardId = memoryStewardCombo->currentData().toString().trimmed();
        if (stewardId.isEmpty() || !chatService)
            return;
        Identity* steward = IdentityManager::instance()->findById(stewardId);
        if (!steward || !steward->profile())
            return;
    ...
    auto* heartbeatSvc = chatService->heartbeatService();
    ...
    auto loadHeartbeatUiForSelected = [=]() {
        ...
        if (path.isEmpty())
            path = configService ? configService->agentHeartbeatInstructionPath(agentId) : QString();
        heartbeatInstructionEdit->setProperty("heartbeatPath", path);
        heartbeatPathLabel->setText(path);
        heartbeatInstructionEdit->setPlainText(configService ? configService->readPossiblyMojibakeUtf8File(path) : QString());
        refreshHeartbeatStateUiForSelected();
    };
    ...
    auto applyHeartbeatUiForSelected = [=](bool showToast) -> bool {
        ...
        if (!configService || !configService->writeUtf8TextFile(path, heartbeatInstructionEdit->toPlainText(), &heartbeatWriteError)) {
            if (showToast) {
                QMessageBox::warning(
                    parent,
                    QObject::tr("保存失败"),
    ...
    auto* schedulerSvc = chatService->schedulerService();
    ...
    QObject::connect(memoryReindexBtn, &QPushButton::clicked, &dlg, [=]() {
        ...
        const bool ok = chatService->rebuildMemoryIndexAs(userId, QString(), &rebuildResult, &rebuildError);
