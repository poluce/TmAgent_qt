...
{
    const QJsonObject policy = configService->normalizeToolLoopPolicyObject(src);
...
{
    ConfigService* configService = chatService ? chatService->configService() : nullptr;
    if (!configService) {
    ...
    loadShellPolicyEditors(ShellTool::loadPolicyObject(),
                           ...
                           dangerEdit,
                           writeEdit);
    loadToolLoopEditors(configService->loadToolLoopPolicyObject(),
                        configService,
                        maxToolRoundsSpin,
    ...
    QObject::connect(resetBtn, &QPushButton::clicked, &dlg, [=]() {
        loadShellPolicyEditors(ShellTool::defaultPolicyObject(),
                               ...
                               dangerEdit,
                               writeEdit);
        loadToolLoopEditors(configService->defaultToolLoopPolicyObject(),
                            configService,
                            maxToolRoundsSpin,
                            maxSameToolRoundsSpin,
                            maxNoProgressRoundsSpin,
    ...
    QObject::connect(buttons, &QDialogButtonBox::accepted, &dlg, [=, &dlg]() {
        ...
        QString toolLoopErr;
        if (!configService->saveToolLoopPolicyObject(toolLoopRaw, &toolLoopErr)) {
            QMessageBox::warning(parent,
                                 QObject::tr("保存失败"),
